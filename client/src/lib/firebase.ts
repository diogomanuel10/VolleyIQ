...
/**
 * Em modo dev (sem projecto Firebase) oferecemos um shim que imita a API
 * utilizada pelo `useAuth`. Permite correr a app sem credenciais reais.
 */
interface DevUser {
  uid: string;
  email: string;
...
export function subscribeAuth(cb: (user: User | DevUser | null) => void) {
  if (USE_DEV) {
